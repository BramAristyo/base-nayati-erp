...
import { useProformaDatatable } from '@/composables/sales/useProformaDatatable';
...
const props = defineProps<{
    data: PaginatedResponse<Proforma>;
    filters: PaginateFilter & { start_date?: string; end_date?: string };
}>();

const authStore = useAuthStore();
const { search, startDate, endDate, onPage, onSort, resetFilters, onExport } = useProformaDatatable(props);

const onRowClick = (event: any) => {
    if (authStore.hasPermission('sales.proforma.view')) {
        router.get(route('sales.proformas.show', { id: event.data.id }));
    }
};
</script>
    ...
    <AppLayout>
        <div class="flex flex-col gap-2">
            <div class="flex flex-col gap-4 md:flex-row md:items-center justify-between px-1">
                <AppPageHeader title="Proforma" description="Manage and track proforma invoices." />

                <div class="flex items-center gap-2">
                    <Button v-if="authStore.hasPermission('sales.proforma.export')" icon="pi pi-file-excel"
                    ...
                    <Button v-if="authStore.hasPermission('sales.proforma.create')" icon="pi pi-plus"
                ...
                <StandardDataTable :data="data" :filters="filters" class="cursor-pointer" @page="onPage" @sort="onSort"
                    ...
                    <template #empty>
                        <div class="p-8 text-center text-sm font-medium text-muted-foreground">
                            No proformas found matching your search.
                        </div>
                    </template>

                    <Column field="proforma_number" header="PROFORMA NUM" sortable class="w-48">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ slotProps.data.proforma_number }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="netto" header="TOTAL" sortable class="text-right">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ formatCurrency(slotProps.data.netto) }}</span>
                        </template>
                    </Column>
                </StandardDataTable>
            </div>
