...
import { useDeliveryOrderDatatable } from '@/composables/sales/useDeliveryOrderDatatable';
...
const props = defineProps<{
    data: PaginatedResponse<DeliveryOrder>;
    filters: PaginateFilter & { start_date?: string; end_date?: string };
}>();

const authStore = useAuthStore();
const { search, startDate, endDate, onPage, onSort, resetFilters, onExport } = useDeliveryOrderDatatable(props);

const onRowClick = (event: any) => {
    if (authStore.hasPermission('sales.delivery-order.view')) {
        router.get(route('sales.delivery-orders.show', { id: event.data.id }));
    }
};
</script>
    ...
    <AppLayout>
        <div class="flex flex-col gap-2">
            <div class="flex flex-col gap-4 md:flex-row md:items-center justify-between px-1">
                <AppPageHeader title="Delivery Order" description="Manage and track delivery orders." />

                <div class="flex items-center gap-2">
                    <Button v-if="authStore.hasPermission('sales.delivery-order.export')" icon="pi pi-file-excel"
                    ...
                    <Button v-if="authStore.hasPermission('sales.delivery-order.create')" icon="pi pi-plus"
                ...
                <StandardDataTable :data="data" :filters="filters" class="cursor-pointer" @page="onPage" @sort="onSort"
                    ...
                    <Column field="delivery_order_number" header="DO NUM" sortable class="w-48">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ slotProps.data.delivery_order_number }}</span>
                        </template>
                    </Column>
