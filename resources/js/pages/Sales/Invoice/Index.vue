...
import { useInvoiceDatatable } from '@/composables/sales/useInvoiceDatatable';
...
const { search, startDate, endDate, onPage, onSort, resetFilters, onExport } = useInvoiceDatatable(props);

const onRowClick = (event: any) => {
    if (authStore.hasPermission('sales.invoice.view')) {
        router.get(route('sales.invoices.show', { id: event.data.id }));
    }
};
</script>
    ...
    <AppLayout>
        <div class="flex flex-col gap-2">
            <div class="flex flex-col gap-4 md:flex-row md:items-center justify-between px-1">
                <AppPageHeader title="Invoice" description="Manage and track customer invoices." />

                <div class="flex items-center gap-2">
                    <Button v-if="authStore.hasPermission('sales.invoice.export')" icon="pi pi-file-excel"
                    ...
                    <Button v-if="authStore.hasPermission('sales.invoice.create')" icon="pi pi-plus"
            ...
            <div class="overflow-hidden">
                <StandardDataTable :data="data" :filters="filters" class="cursor-pointer" @page="onPage" @sort="onSort"
                    ...
                    <Column field="invoice_number" header="INV NUM" sortable class="w-48">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ slotProps.data.invoice_number }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="total" header="TOTAL" sortable class="text-right">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ formatCurrency(slotProps.data.total) }}</span>
                        </template>
                    </Column>
                </StandardDataTable>
            </div>
