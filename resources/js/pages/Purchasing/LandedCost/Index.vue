    ...
    <AppLayout>
        <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-col gap-4 md:flex-row md:items-center justify-between px-1">
                <AppPageHeader title="Landed Cost"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Button v-if="authStore.hasPermission('purchasing.landed-cost.create')" icon="pi pi-plus"
                ...
                <StandardDataTable :data="data" :filters="filters" class="cursor-pointer" @page="onPage" @sort="onSort"
                    ...
                    <Column field="landed_cost_number" header="LANDED COST NUM" sortable class="w-48">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ slotProps.data.landed_cost_number
                                }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="receiving_number" header="RECEIVING NUMBER" sortable class="w-48">
                        <template #body="slotProps">
                            <span class="text-[11px] font-semibold text-foreground">{{ slotProps.data.receiving_number
                                }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="currency_code" header="CURRENCY" class="w-24 text-center">
                        <template #body="slotProps">
                            <span class="text-[10px] font-bold uppercase text-muted-foreground">{{
                                slotProps.data.currency_code }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="sub_total" header="EXWORK" sortable class="text-right w-36">
                        <template #body="slotProps">
                            <span class="text-xs font-bold text-foreground">{{ slotProps.data.sub_total.toLocaleString()
                                }}</span>
                        </template>
                    </Column>
