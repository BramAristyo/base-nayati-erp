...
import PurchasingApprovalTabs from '@/components/Approvals/PurchasingApprovalTabs.vue';
...
<template>
    ...
    <AppLayout>
        <div class="flex flex-col gap-2 h-[calc(100vh-8rem)]">
            ...
            <!-- Header -->
            <div class="flex flex-col gap-4 md:flex-row md:items-center justify-between px-1 shrink-0">
            ...
            <!-- Module Tabs -->
            <PurchasingApprovalTabs activeTab="PR" />

            <Divider class="my-1!" />

            <!-- Filter Strip -->
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-1 shrink-0">
            ...
            <div class="flex-1 min-h-0 overflow-hidden mx-1">
                <Splitter stateKey="pr-approval-splitter" layout="vertical"
                    ...
                    <SplitterPanel :size="55" :minSize="30"
                        ...
                        <DataTable :key="routeName" :value="data.data" v-model:selection="selectedItems" dataKey="id"
                            lazy paginator :rows="data.per_page" :rowsPerPageOptions="[10, 25, 50, 100]"
                            :totalRecords="data.total" :first="(data.current_page - 1) * data.per_page"
                            :sortField="filters.sortField" :sortOrder="Number(filters.sortOrder) || -1"
                            @page="onPage" @sort="onSort" @row-click="handleRowClick"
                            :rowClass="(d: any) => activeRow?.id === d.id ? 'bg-primary/5!' : ''"
