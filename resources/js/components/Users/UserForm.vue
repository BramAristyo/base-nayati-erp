...
<template>
    <form @submit.prevent="emit('submit')"
        class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-8 rounded-xl shadow-sm border border-gray-100">
        <!-- Basic Information -->
        ...
        <div class="flex flex-col gap-2">
            ...
            <Select id="position" v-model="form.position" :options="positions" optionLabel="label" optionValue="value"
                size="small" :invalid="!!form.errors.position" class="w-full!" placeholder="Select Position" />
            <small v-if="form.errors.position" class="text-red-500 font-medium">{{ form.errors.position }}</small>
        </div>

        <div class="flex flex-col gap-2">
            <label for="branch_code" class="text-xs font-bold uppercase tracking-widest text-gray-700">Branch
                Code</label>
        ...
        <div class="flex flex-col gap-2">
            <label for="approver_name" class="text-xs font-bold uppercase tracking-widest text-gray-700">Approver
                Name</label>
            ...
            <small v-if="form.errors.approver_name" class="text-red-500 font-medium">{{ form.errors.approver_name
            }}</small>
        </div>

        <div class="flex flex-col gap-2">
            <label for="approver_title" class="text-xs font-bold uppercase tracking-widest text-gray-700">Approver
                Title</label>
            ...
            <small v-if="form.errors.approver_title" class="text-red-500 font-medium">{{ form.errors.approver_title
            }}</small>
        </div>
        ...
        <div class="flex flex-col gap-2">
            <label for="warehouses" class="text-xs font-bold uppercase tracking-widest text-gray-700">Warehouse
                Access</label>
        ...
        <template v-if="props.isEdit">
            <div class="flex flex-col gap-2">
                <label for="password" class="text-xs font-bold uppercase tracking-widest text-gray-700">New Password
                    (Optional)</label>
            ...
            <div class="flex flex-col gap-2">
                <label for="password_confirmation"
                    class="text-xs font-bold uppercase tracking-widest text-gray-700">Confirm
                    New Password</label>
                <InputText id="password_confirmation" v-model="form.password_confirmation" type="password" size="small"
                    class="w-full!" />
            </div>
        </template>
        ...
        <div class="flex items-center gap-4 col-span-1 md:col-span-2 mt-2">
            <div class="flex flex-col">
                ...
                <span class="text-[10px] text-gray-500 font-medium">Toggle to activate or deactivate this user
                    account.</span>
            </div>
        ...
        <div class="flex justify-end gap-3 col-span-1 md:col-span-2 mt-6 pt-6 border-t border-gray-100">
            <Button label="Cancel" severity="secondary" variant="text" size="small"
                class="font-bold! uppercase! tracking-widest!" @click="confirmCancel" />
            <Button type="submit" :label="isEdit ? 'Update User' : 'Save User'" icon="pi pi-check" size="small"
                :loading="form.processing"
                class="bg-black! border-none! text-white! font-bold! uppercase! tracking-widest! px-6! shadow-md!" />
        </div>
    </form>
</template>
