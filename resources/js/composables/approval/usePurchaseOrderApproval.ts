...
import type { PurchaseOrderItem } from '@/types/purchasing/purchase-order-item.types';
...
export function usePurchaseOrderApproval(props: {
    ...
    const selectedItems = ref<PurchaseOrder[]>([]);
    const activeRow = ref<PurchaseOrder | null>(null);
    const detailItems = ref<PurchaseOrderItem[]>([]);
    const isLoadingDetails = ref(false);

    const routeName = computed(() =>
        currentStatus.value === 'pending'
            ? 'approval.purchasing.purchase-orders.pending'
            : 'approval.purchasing.purchase-orders.processed'
    );
    ...
    const loadDetail = async (row: PurchaseOrder) => {
        ...
        try {
            const response: any = await http.get(
                route('api.purchasing.purchase-orders.show', { id: row.id })
            );
            if (response.status) {
