...
export function useReceivingDatatable(props: { data: PaginatedResponse<any>; filters: any }) {
    ...
    const { search, onPage, onSort, updateRoute } = useDataTable({
        routeName: 'purchasing.receivings.index',
    ...
    const onExport = () => {
        ...
        window.location.assign(route('purchasing.receivings.export', params));
    };
