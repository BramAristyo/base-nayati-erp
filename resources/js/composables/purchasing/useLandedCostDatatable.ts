...
export function useLandedCostDatatable(props: { data: PaginatedResponse<any>; filters: any }) {
    ...
    const { search, onPage, onSort, updateRoute } = useDataTable({
        routeName: 'purchasing.landed-costs.index',
    ...
    const onExport = () => {
        ...
        window.location.assign(route('purchasing.landed-costs.export', params));
    };
