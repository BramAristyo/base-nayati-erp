...
export function useShipmentDatatable(props: { data: PaginatedResponse<any>; filters: any }) {
    ...
    const { search, onPage, onSort, updateRoute } = useDataTable({
        routeName: 'sales.shipments.index',
    ...
    const onExport = () => {
        ...
        window.location.assign(route('sales.shipments.export', params));
    };
