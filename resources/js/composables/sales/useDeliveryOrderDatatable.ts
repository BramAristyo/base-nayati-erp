...
export function useDeliveryOrderDatatable(props: { data: PaginatedResponse<any>; filters: any }) {
    ...
    const { search, onPage, onSort, updateRoute } = useDataTable({
        routeName: 'sales.delivery-orders.index',
    ...
    const onExport = () => {
        ...
        window.location.assign(route('sales.delivery-orders.export', params));
    };
