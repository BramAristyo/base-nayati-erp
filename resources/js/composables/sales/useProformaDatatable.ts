...
export function useProformaDatatable(props: { data: PaginatedResponse<any>; filters: any }) {
    ...
    const { search, onPage, onSort, updateRoute } = useDataTable({
        routeName: 'sales.proformas.index',
    ...
    const onExport = () => {
        ...
        window.location.assign(route('sales.proformas.export', params));
    };
